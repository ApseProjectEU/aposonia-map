class StationMap {
  ...
  init() { this.createMap(); this.addMarkers(); this.createPanels(); this.attachCloseHandlers(); }
  createMap() {
  ...
  addMarkers() {
    this.stations.forEach(station => {
      const icon = L.divIcon({ className: 'custom-marker', html: `<span class="marker-number">${station.id}</span>`, iconSize: [40, 40] });
      const marker = L.marker(station.coords, { icon }).addTo(this.map);
      marker.on('click', () => this.openStation(station.id));
      this.markers.push(marker);
    });
  }
  createPanels() {
    const container = document.getElementById('station-panels');
    const overlay = document.createElement('div');
    overlay.className = 'panel-overlay';
    overlay.addEventListener('click', () => this.closeAllStations());
    container.appendChild(overlay);
    this.stations.forEach(s => container.appendChild(this.createPanelHTML(s)));
  }
  createPanelHTML(station) {
    ...
    panel.innerHTML = `
      ...
      <div class="station-header">
        ...
        <p class="station-location" data-i18n="${station.locationKey}"></p>
      </div>
      <div class="audio-player"><audio controls preload="metadata"><source src="${station.audioPath}" type="audio/mpeg"></audio></div>
      <div class="curatorial-text" data-i18n="stations.station${station.id}.text"></div>
      <div class="metadata"><dl>
        <dt>Recorded</dt><dd data-i18n="stations.station${station.id}.date"></dd>
        <dt>Depth</dt><dd data-i18n="stations.station${station.id}.depth"></dd>
        <dt>Conditions</dt><dd data-i18n="stations.station${station.id}.conditions"></dd>
      </dl></div>`;
    return panel;
  }
  openStation(id) {
  ...
  closeAllStations() {
    document.querySelectorAll('.station-panel').forEach(p => { p.classList.remove('active'); p.querySelector('audio')?.pause(); });
    document.querySelector('.panel-overlay')?.classList.remove('active');
    document.body.style.overflow = '';
  }
  attachCloseHandlers() {
    document.addEventListener('click', e => { if (e.target.closest('.close-panel')) this.closeAllStations(); });
    document.addEventListener('keydown', e => { if (e.key === 'Escape') this.closeAllStations(); });
  }
}
